 ...
 ************************************************************************************** */
package org.calypsonet.keyple.demo.control.mock
  ...
  fun getMockedResult(context: Context, status: Status): CardReaderResponse? {
    ...
    when (status) {
      Status.TICKETS_FOUND -> {
        val parser = SimpleDateFormat(DateUtil.yyyy_MM_dd_HH_mm_ss, Locale.getDefault())
        ...
        cardReaderResponse =
            CardReaderResponse(
                ...
                arrayListOf(
                    Contract(
                        name = ContractPriority.MULTI_TRIP.value,
                        nbTicketsLeft = 2,
                        validationDate = null,
                        valid = true,
                        record = 1,
                        expired = false,
                        contractValidityStartDate = DateTime(DateUtil.parseDate("01/01/2021")),
                        contractValidityEndDate = DateTime(DateUtil.parseDate("01/01/2030"))),
                    Contract(
                        name = ContractPriority.SEASON_PASS.value,
                        valid = false,
                        validationDate = null,
                        record = 2,
                        expired = true,
                        contractValidityStartDate = DateTime(DateUtil.parseDate("01/04/2020")),
                        contractValidityEndDate = DateTime(DateUtil.parseDate("01/08/2020")))))
      }
      Status.LOADING, Status.ERROR, Status.SUCCESS, Status.INVALID_CARD, Status.EMPTY_CARD -> {
        ...
        cardReaderResponse =
            CardReaderResponse(status = status, titlesList = arrayListOf(), errorMessage = error)
      }
      Status.WRONG_CARD -> {
        // Do nothing
      }
